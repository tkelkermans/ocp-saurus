...
8. Cat out the contents of ``rootCA.crt``, ``pc.key`` and ``pc.crt`` 
...
13. Click on **Import Files**
    ...
    Contact your instruction if you need help with troubleshooting SSL certificate issues.
...
5. Add the details:
   ...
   ```
   ```buttonless {4-15,21,37} title="Sample install-config.yaml file. You can also reduce the number of workers to 2"
   apiVersion: v1
